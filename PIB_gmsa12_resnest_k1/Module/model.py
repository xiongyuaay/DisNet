...
class DisNet(nn.Module):
    def __init__(self, args) -> None:
        ...
        for i in range(self.Module2_depth):
            if i % 2 != 0:
                moudle_GMSA['gmsa_{}'.format(i+1)] = GMSA(channels=self.channels, shifts=0, window_sizes=self.window_sizes, calc_attn=False)
            else:
                moudle_GMSA['gmsa_{}'.format(i+1)] = GMSA(channels=self.channels, shifts=1, window_sizes=self.window_sizes, calc_attn=False)
        self.gmsa = nn.ModuleDict(moudle_GMSA)
    ...
    def _forward_impl(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.Module2_depth):
            ...
            y, attn = self.gmsa['gmsa_{}'.format(i+1)](out1, attn)
            out1 = torch.add(y, out1)
