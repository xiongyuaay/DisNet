...
def compute_iiw_bp(model, train_dataloader, param_list, no_bp=False, device='cuda'):
    ...
    for _ in range(10):
        ...
        psd1D_img = np.zeros([pred.shape[0], N])
        for t in range(pred.shape[0]):
            gen_imgs = pred.permute(0,2,3,1)
        ...
        psd1D_rec = np.zeros([y_batch.shape[0], N])
        for t in range(y_batch.shape[0]):
            gen_imgs = y_batch.permute(0,2,3,1)
            img_numpy = gen_imgs[t,:,:,:].cpu().detach().numpy()
            img_gray = RGB2gray(img_numpy)
            fft = np.fft.fft2(img_gray)
            fshift = np.fft.fftshift(fft)
        ...
        loss = loss + lambda_freq*loss_freq
